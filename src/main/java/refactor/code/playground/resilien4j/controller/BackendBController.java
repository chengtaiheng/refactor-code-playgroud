...
public class BackendBController {
    ...
    private static final String BACKEND_B = "backenB";
    ...
    private <T> Mono<T> execute(Mono<T> publisher) {
        return publisher.transform(BulkheadOperator.of(bulkhead))
    ...
    private Mono<String> monoFallback(Throwable throwable) {
        return Mono.just("Recovered: " + throwable.toString());
    }


    private Flux<String> fluxFallback(Throwable throwable) {
        return Flux.just("Recovered:" + throwable.toString());
    }


    private <T> CompletableFuture executeAsync(Supplier<T> supplier) {
        return Decorators.ofSupplier(supplier)
    ...
    private <T> CompletableFuture<T> executeAsyncWithFallback(Supplier<T> supplier, Function<Throwable, T> fallback) {
        return Decorators.ofSupplier(supplier)
                ...
                .withTimeLimiter(timeLimiter, scheduledExecutorService)
                .withCircuitBreaker(circuitBreaker)
                .withFallback(asList(TimeoutException.class, CallNotPermittedException.class, BulkheadFullException.class), fallback)
                .get().toCompletableFuture();
    }

    private String fallback(Throwable throwable) {
        return "Recovered: " + throwable.toString();
    }
}
